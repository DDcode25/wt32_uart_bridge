...
void config_manager_apply_profile(app_config_t *cfg, config_profile_t profile)
{
    ...
    switch (profile) {
        ...
        case PROFILE_E_CRSF_LINK_HANDSET:
             ...
             *
             * 2026-08-19: перебран весь диапазон темпа — 13, 8, 6, 4 и 2
             * кадра в секунду. Окна не существует: на 13 пульт зависает, на
             * 8 уходит в Emergency mode, на 2 живёт, но телеметрию не
             * удерживает. Доля искажённых посылок при этом около 45% и от
             * темпа НЕ зависит — значит портится каждая посылка сама по
             * себе, а не линия от загрузки. Содержимое, коллизии и адрес
             * назначения исключены по отдельности, отказ повторялся при
             * всех. Предел электрический, настройками не обходится;
             * подробности и таблица — в README. */
            cfg->routing[1].net_to_uart = false;
            break;
        ...
        case PROFILE_F_CRSF_LINK_TRANSMITTER:
             ...
             *
             * ВЫВОДЫ ЛЕГКО ПЕРЕПУТАТЬ, и на этом стенде это случалось трижды.
             * Канал при этом выглядит полностью настроенным и молча ничего не
             * принимает, потому что вывод приёма подключён к приёму же
             * устройства. Замерено на обеих ориентациях: на верной 3752 байта
             * чистого CRSF за 20 секунд, на обратной — 2 байта.
             *
             * Опознаётся за два замера, без осциллографа: перевести канал в
             * протокол RAW, снять запись в провод, направить поток на ПК и
             * посмотреть на гистограмму значений байт.
             *   все 256 значений, много 0x00 и 0xFF -> вход болтается,
             *                                          провод не туда;
             *   узкий набор с ровным периодом       -> распайка верна, промах
             *                                          по полярности или скорости;
             *   тишина                              -> на этом выводе
             *                                          передатчика нет.
             *
             * Обе ориентации рабочие, если настройку привести в соответствие
             * проводам. Приём на GPIO5 читался чуть чище (CRC 4 и sync 24 за
             * 15 секунд против 9 и 60 на GPIO17), разница мелкая. */
            cfg->uart[1].protocol   = PROTO_MODE_CRSF;
             ...
             * потоком. Выводы 32/33, потому что 17 и 5 заняты CRSF выше.
             * Адресат задан явно, иначе поток ждёт, пока наземная станция
             * напишет первой. */
            cfg->uart[2].protocol   = PROTO_MODE_MAVLINK;
...
char *config_manager_to_json(const app_config_t *cfg)
{
    ...
    for (int i = 0; i < UART_MGR_NUM_CHANNELS; i++) {
        ...
        cJSON_AddBoolToObject(c, "net_to_uart_unchecked", cfg->routing[i].net_to_uart_unchecked);
...
esp_err_t config_manager_from_json(const char *json, app_config_t *out)
{
    ...
    if (cJSON_IsArray(chans)) {
        ...
        for (int i = 0; i < n && i < UART_MGR_NUM_CHANNELS; i++) {
            ...
            out->routing[id].net_to_uart_unchecked =
                json_bool(c, "net_to_uart_unchecked", out->routing[id].net_to_uart_unchecked);
