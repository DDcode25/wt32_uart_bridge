...
/* Пины программирования/консоли. Это родные выводы UART0 (TX0=GPIO1,
 * RX0=GPIO3), поэтому канал 0 может работать на них напрямую, без
 * перекладывания GPIO-матрицей. Валидация их пропускает, но прошивка
 * предупреждает при старте канала: подключённое сюда устройство мешает
 * заливке через USB-UART, и загрузочный лог уходит в него же. */
#define BOARD_PROG_RESERVED_GPIOS { 1, 3 }
#define BOARD_CONSOLE_IO_GPIOS    { 1, 3 }
