...
/* То же самое, но БЕЗ проверки кадра.
 *
 * Нужно ровно для прозрачного режима, где мост не разбирает содержимое и
 * не вправе судить о нём: что пришло из сети, то и уходит в провод. Срок
 * годности и вытеснение старого работают и здесь — они относятся к линии,
 * а не к содержимому. Для обычного режима не использовать: в общую шину не
 * должно попадать то, что не сошлось по длине и CRC. */
